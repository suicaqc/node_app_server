(function () { 
		var obj =  function (pkg, req, res) {
			this.post = function() {
				...
				var db 	= {
					cache 	: new pkg.Nedb({ filename: __dirname + '/_db/post_cache.db', autoload: true }),
					auth	: new pkg.Nedb({ filename: __dirname + '/_db/auth.db', autoload: true })
				}; 
				var _f = {};
			...
			this.get = function() {
				var CP = new pkg.crowdProcess();
				var db 	= {
					cache 	: new pkg.Nedb({ filename: __dirname + '/_db/get_cache.db', autoload: true })
				}; 
